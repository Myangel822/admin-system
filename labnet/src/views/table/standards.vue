...
<script>
import {
  getStandards, deleteStandards, getStandardsByName, editStandardState
} from '@/api/table'
  ...
  methods: {
    fetchData() {
      ...
      getStandards().then(response => {
        ...
        this.total = response.data.lists.length
      })
    },
    searchData(bool) {
      this.currentPage = 1
      if (bool) {
        getStandardsByName(this.inputVal).then(res => {
    ...
    editRows(id) {
      this.$router.push({ name: 'EditStandards', query: {
        id: id
      }})
    },
