    ...
    >
      ...
      <el-table-column prop="img" label="图片" width="200">
        <template v-slot:default="scope">
          <el-image :src="('/static/dist/' + scope.row.name)" alt="" />
        </template>
      </el-table-column>
...
<script>
import { getImagesByDate, getImages, deleteImage } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    searchData(bool) {
      this.currentPage = 1
      if (bool) {
        getImagesByDate(this.inputVal).then(res => {
    ...
    editRows(id) {
      this.$router.push({ name: 'EditImage', query: {
        id: id
      }})
    },
